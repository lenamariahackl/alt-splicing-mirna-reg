...
def check_args(input_path, bindingsites, log_path):
    ...
    if not bindingsites:
        if not input_path or not os.path.exists(input_path):
            ...
            exit()
        parquet_file = Path(input_path/'bs_subset.parquet')
...
#put seperate bindingsite files back together and save to parquet 
def combine_seperate_files(path):
    all_chrom = []
    for i in range(1,10):
...
def main(input_path="", bindingsites=False, log_path=""):
    ...
    combine_seperate_files(input_path)
